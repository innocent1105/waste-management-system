...
export default function UserGps() {
    ...
    useEffect(() => {
        const fetchTruck = async () => {
            ...
            console.log(user_id)
            try {
                const res = await axios.post(`${API_BASE_URL}/user_gps.php`, {
                    user_token : user_id
                });
                            ...
                            <div className="card-header">
                                ...
                                <div className="status-text-group">
                                    <h2 className="status-title">
                                        {activeOrder ? "Waste Collection" : "Waste Collection"}
                                    </h2>
                                    <p style={{fontSize: '13px', color: '#666', margin: 0}}>
                                        {activeOrder ? "Route is accurate" : "Make sure your network is stable"}
                                    </p>
                                </div>
                            </div>
