...
import { ArrowLeft, Search, Filter, Wallet, ArrowUpRight, ArrowDownLeft, ChevronRight, X } from 'lucide-react';
...
export default function AdminTransactions({ }) {
  ...
  const currency = "K";
  const categories = ["All", "Tech", "Travel", "Gadgets", "Office", "Food", "Income"];
      ...
      <div className="trx-content-layout">
        <aside className="trx-insights">
          <div className="balance-card">
            ...
            <div className="card-footer"><span>ADMIN ACCOUNT</span><Wallet size={20}/></div>
          </div>

          <div className="mini-stats">
            <div className="m-stat">
              <div className="m-icon up"><ArrowUpRight size={16}/></div>
              <div><p>Income</p><strong>{currency}{stats.income.toLocaleString()}</strong></div>
            </div>
            <div className="m-stat">
              <div className="m-icon down"><ArrowDownLeft size={16}/></div>
              <div><p>Failed</p><strong>{currency}{Math.abs(stats.spending).toLocaleString()}</strong></div>
            </div>
          </div>
        ...
        <main className="trx-main-feed">
          <div className="feed-header">
            <h3>{activeCat} Transactions ({transactions.length})</h3>
          </div>
          ...
          <div className="trx-list">
            {loading ? (
              ...
              <div key={t.id} className="trx-row" onClick={() => onSelectTransaction(t)}>
                ...
                <div className="trx-amount-cell">
                  <h4 className={t.status === 'SUCCESS' ? 'positive' : ''}>
